 ...
 *
 * Storage key is configurable per course via initProgress()
 */

import { atom, computed } from 'nanostores';
import type { UserProgress, ChapterProgress, UserSettings, CheckpointProgress, Bookmark } from './types';

// Current storage key (set via initProgress)
let STORAGE_KEY = 'book-progress';
 ...
 * @param courseId - Unique identifier for the course (used as storage key prefix)
 */
export function initProgress(courseId?: string): void {
  if (courseId) {
    STORAGE_KEY = `${courseId}-book-progress`;
  }
...
/**
 * Create a computed store for overall course progress
 * @param totalChapters - Total number of chapters in the course
 */
export function createCourseProgress(totalChapters: number) {
  return computed(progressStore, (progress) => {
    const chapters = Object.values(progress.chapters);
    if (chapters.length === 0) return 0;

    const totalProgress = chapters.reduce((sum, ch) => sum + (ch.readProgress || 0), 0);
    return Math.round(totalProgress / totalChapters);
  });
}
