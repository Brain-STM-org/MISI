...
export default function ConceptReviewSidebar({ chapterSlug }: Props) {
  ...
  return (
    <aside className="concept-review-sidebar fixed bottom-4 right-4 z-30" aria-label="Key concepts review">
      {/* Collapsed button */}
      {!isExpanded && (
        <button
          onClick={() => setIsExpanded(true)}
          className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-full shadow-lg hover:bg-purple-700 transition-colors focus-ring"
          aria-expanded="false"
          aria-label={`Review ${concepts.length} key concepts`}
        >
        ...
        <div className="w-80 max-h-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-purple-50 dark:bg-purple-900/30 border-b border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 focus-ring rounded"
              aria-label="Close concepts panel"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {/* Concept list as accordion */}
          <div className="overflow-y-auto max-h-72" role="region" aria-label="Concept list">
            {concepts.map((concept, index) => {
              const isConceptExpanded = expandedConcepts.has(concept.id);
              const panelId = `concept-panel-${concept.id || index}`;
              const headerId = `concept-header-${concept.id || index}`;
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full px-4 py-3 text-left hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors focus-ring"
                  ...
                  {isConceptExpanded && (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-xs text-blue-600 dark:text-blue-400 hover:underline focus-ring"
                      >
                        Jump to concept
                      </button>
                    </div>
          ...
          <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900 text-xs text-gray-500 dark:text-gray-400">
            Click to expand, then jump to review in context. Press Escape to close.
          </div>
        </div>
      )}
    </aside>
