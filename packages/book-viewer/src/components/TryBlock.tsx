...
function extractSlots(children: ReactNode): { exercise: ReactNode; hint: ReactNode } {
  ...
  Children.forEach(children, (child) => {
    if (isValidElement(child)) {
      const slot = (child.props as Record<string, unknown>)?.slot;
      if (slot === 'exercise') {
        exercise = (child.props as Record<string, unknown>).children || child;
      } else if (slot === 'hint') {
        hint = (child.props as Record<string, unknown>).children || child;
      } else {
      ...
      <div className="p-4">
        ...
        {hasHintContent && hint && (
          <div className="try-hint-section mt-4">
            ...
            >
              <span aria-hidden="true">Need a hint?</span>
            </button>
            <div
              id="try-hint-content"
